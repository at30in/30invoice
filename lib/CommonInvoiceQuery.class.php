...
class CommonInvoiceQuery extends Doctrine_Query
{
  ...
  public function search($search = null)
  {
    if($search)
    {
      ...
      if(isset($search['customer_code'])) $this->customer($search['customer_code']);
  ...
  public function series_text($text)
  {
    $text = trim($text);
    if($text)
    {
      $this->leftJoin("i.Series s");
      $this->andWhere("i.number LIKE ?", "%$text%");
      $this->orWhere("s.value LIKE ?", "%$text%");
    }
