...
/**
 * customers requisitions actions.
 *
 * @package    siwapp
 * @subpackage invoices
 * @author     Siwapp Team
 ...
 */
class customersRequisitionsActions extends sfActions
{
  ...
  {
    $this->forward404Unless($customer = Doctrine::getTable('CustomerRequisition')->find($request->getParameter('id')),
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    $q = CustomerRequisitionQuery::create()->search($search)->orderBy("$sort[0] $sort[1], name $sort[1]");
    $date_range = array();
    ...
    $this->pager = new sfDoctrinePager('CustomerRequisition', $maxResults);
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    $customer = new CustomerRequisition();
    
    $this->customerRequisitionForm = new CustomerRequisitionForm($customer, array('culture'=>$this->culture));
  ...
  public function executeCreate(sfWebRequest $request)
  {
    ...
    $this->customerRequisitionForm = new CustomerRequisitionForm(null, array('culture' => $this->culture));
    $this->title = $this->getContext()->getI18N()->__('New Customer');
    $this->action = 'create';

    $this->processForm($request, $this->customerRequisitionForm);
    $this->setTemplate('edit');
  }
  ...
  public function executeDelete(sfWebRequest $request)
  {
    ...
    $this->redirect('customersRequisitions/index');
  }
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      $this->redirect('customersRequisitions/edit?id='.$customer->id);
    }
    else
    {
  ...
  public function executeBatch(sfWebRequest $request)
  {
       ...
       $this->getContext()->getConfiguration()->getEnvironment() == 'test')
    {
      ...
      {
        if($customer = Doctrine::getTable('CustomerRequisition')->find($id))
        {
    ...
    $this->redirect('@customers_requisitions');
  }
